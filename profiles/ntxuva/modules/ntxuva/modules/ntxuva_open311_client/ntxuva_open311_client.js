...
(function ($) {
  ...
  Drupal.behaviors.ntxuvaOpen311Client = {

    attach: function (context, settings) {
      Drupal.Ntxuva = Drupal.Ntxuva || {};
      var map = Drupal.Ntxuva.maps[0];
      Drupal.ntxuvaOpen311Client.map = Drupal.ntxuvaOpen311Client.createMap(Drupal.Ntxuva.maps[0].getCenter());

      // Define layer:
      addMarkerLayer = new L.LayerGroup();

      map.on('click', function (e) {
        Drupal.ntxuvaOpen311Client.setMapMarker(e.latlng, 0);
      });

      $('#open311_client').on('shown.bs.modal', function (e) {
        Drupal.ntxuvaOpen311Client.codeLatLng(Drupal.Ntxuva.maps[0].getCenter(), 0, 'marker');
        // We need this to make map load completely.
        setTimeout(function () {
          Drupal.ntxuvaOpen311Client.map.invalidateSize();
        }, 10);
      });
      ...
      L.easyButton('fa fa-edit', 'topright',
        function () {
          ...
          Drupal.ntxuvaOpen311Client.setMapMarker(map.getCenter(), 0);
        },
  ...
  Drupal.ntxuvaOpen311Client = {
    setMapMarker: function (latLng) {

      // Add Layergroup and define ImagePath.
      L.Icon.Default.imagePath = Drupal.settings.ntxuvaOpen311Client.ntxuva_open311_client_leafletImagePath;

      // Remove old marker.
      if (Drupal.Ntxuva.maps[0].hasLayer(addMarkerLayer) == true) {
        addMarkerLayer.clearLayers();
      }
      // Remove list items
      $('.main-container').fadeOut();
      Drupal.Ntxuva.maps[0].setView(latLng);
      ...
      marker.on('click', function (e) {
        Drupal.ntxuvaOpen311Client.showForm(marker);
      });
    ...
    codeLatLng: function (latLng, i, op) {
      var settings = Drupal.settings.ntxuvaOpen311Client;
      var nominatimEmail = settings.ntxuva_open311_client_nominatimEmail;
      var url = settings.ntxuva_open311_client_geocode_service + '/reverse';
    ...
    createMap: function (latlng) {
      ...
      switch (mapType) {
        case '0':
          var attribution = mopa.osm_custom_attribution;
          var layer = new L.Google('ROADMAP');
          break;
        case '1':
          var tiles = 'https://{s}.tiles.mapbox.com/v3/' + mopa.mapbox_map_id + '/{z}/{x}/{y}.png';
      ...
      zoom.sync(Drupal.Ntxuva.maps[0]);
      // Update lat lng form value
      $(zoom).on('dragend', function (e) {
        Drupal.ntxuvaOpen311Client.codeLatLng(Drupal.Ntxuva.maps[0].getCenter(), 0, 'marker');
        latlng = Drupal.Ntxuva.maps[0].getCenter();
      });

      $(zoom).on('moveend', function (e) {
        Drupal.ntxuvaOpen311Client.codeLatLng(zoom.getCenter(), 0, 'marker');
      });

      return zoom;
    },
    showForm: function (marker) {
      var map = Drupal.ntxuvaOpen311Client.map;
      map.setView(marker.getLatLng(), 16);
      var settings = Drupal.settings.ntxuvaOpen311Client;
      Drupal.ntxuvaOpen311Client.codeLatLng(marker.getLatLng(), 0, 'marker');

      $('#open311_client').modal('show');
      var latlng = marker.getLatLng();

      var select = $.get(settings.ntxuva_open311_client_endpoint + '/services.json', function (services) {
        ...
        Drupal.ntxuvaOpen311Client.localStorage();
      });
      ...
      $("#dropbox, #multiple").html5_upload({
        fieldName: "image",
        url: settings.ntxuva_open311_image_api_url,
        headers: {
          'Authorization': 'Client-ID' + ' ' + settings.ntxuva_open311_image_api_key
        },
        onError: function () {
      ...
      $("#open311_client_submit").click(function (e) {
        ...
        Drupal.ntxuvaOpen311Client.unhighlight();

        $('#lat').val(latlng.lat);
        $('#long').val(latlng.lng);
        $.post(settings.ntxuva_open311_client_endpoint + "/requests.json", $("#open311_client_form").serialize())
          .success(function () {
            spinner.stop();
            Drupal.ntxuvaOpen311Client.success($('textarea'));
          })
          .error(function (response) {
            ...
            if (error.search("email") >= 0) {
              Drupal.ntxuvaOpen311Client.highlight($('input[name="email"]'), error);
            } else if (error.search("position") >= 0) {
              Drupal.ntxuvaOpen311Client.highlight($('input[name="address_string"]'), error);
              $('input[name="address"]').removeAttribute('disabled');
            } else if (error.search("service") >= 0) {
              Drupal.ntxuvaOpen311Client.highlight($('select'), error);
            }
          });
      });
    }
